...
experiment_name = "new_norms_difference_vectors"
repetitions = 100
...
function bifurcation_cut(w_min, w_max, steps)
    ...
    for w in range(w_min, stop=w_max, length=steps)
        ...
        difference_guess = diff(guess, dims=2)
        difference_target = diff(y, dims=2)
        N = length(difference_guess)

        push!(mse, mse_loss([0.0, w, 0.0], y, predict)[1])
        push!(mse_diff, sum(abs2, difference_target .- difference_guess))

        push!(nld, 1/N * sum([normed_ld(a, b) for (a,b) in zip(eachcol(difference_guess), eachcol(difference_target))]))
        push!(cnld, 1/N * sum([cos_normed_ld(a, b) for (a,b) in zip(eachcol(difference_guess), eachcol(difference_target))]))
        push!(scnld, 1/N * sum([sqrt_cos_normed_ld(a, b) for (a,b) in zip(eachcol(difference_guess), eachcol(difference_target))]))

        push!(cs, 1/N * sum([cosine_similarity(a, b) for (a,b) in zip(eachcol(difference_guess), eachcol(difference_target))]))
        push!(cd, 1/N * sum([cosine_distance(a, b) for (a,b) in  zip(eachcol(difference_guess), eachcol(difference_target))]))
        push!(acd, 1/N * sum([angular_cosine_distance(a, b) for (a,b) in zip(eachcol(difference_guess), eachcol(difference_target))]))
        # push!(scd, sum([sqrt_cosine_distance(a, b) for (a,b) in zip(eachcol(difference_guess), eachcol(difference_target))]))
    end

    return (mse, mse_diff, nld, cnld, scnld, cs, cd, acd, scd)
end

(mse, mse_diff, nld, cnld, scnld, cs, cd, acd, scd) = bifurcation_cut(-0.5, 0.5, 1001)
...
mse_line = lines!(ax1, x_range, mse, color = :red)
mse_diff_line = lines!(ax1, x_range, mse_diff, color = :blue)
ax1.xlabel = "w_y"
ax1.ylabel = "Loss MSE"
...
# leg_lengths = legend!(ax2, 
#                      [nld_line, cnld_line, scnld_line], 
#                      ["Normalized Vector Length Distance",
#                       "Cosine-Normalized Vector Length Distance",
#                       "Squareroot of Normalized Vector Length Distance"]
#                     )
ax2.xlabel = "w_y"
